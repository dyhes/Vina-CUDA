	...
	)
{
	...
	while (true)
	{
		...
		*f = m_eval_deriv(x,
		...
		{
			// quickvina-W already checks the Wolfe condition
...
// Zhang-Hager condition
__device__ int line_search_zh(			m_cl* 			m_cl_gpu,
	                        							pre_cl* 		p_cl_gpu,  // delete the constant 
														grids_cl* 		ig_cl_gpu, // delete the constant
														int 			n,
														flo* 			stp,
...
__device__ int line_search_zh_update(			m_cl* 			m_cl_gpu,
	                        							pre_cl* 		p_cl_gpu,  // delete the constant 
														grids_cl* 		ig_cl_gpu, // delete the constant
														int 			n,
														flo* 			stp,
	...
	)
{
	...
	while (true)
	{
		...
		*f = m_eval_deriv_update(x,
		...
		flo factor_1 = 1 / (count * log10f(count/n + 12));
		flo factor_2 = c - (*f);
		flo factor_3 = tau * (*stp) * (dgtest);

		n_k = 1e16;
		if(n_k > factor_1) n_k = factor_1;
		if(n_k > factor_2) n_k = factor_2;
		if(n_k > factor_3) n_k = factor_3;
	}
}
...
__device__ void rilc_bfgs_update(			output_type_cl* 	x,
						...
						const				int					threadsPerBlock,
										...
										individual_container* circularvisited,
										int search_depth,
										vec3_cl* origin,
										vec3_cl* box_size,
										ele_cl* global_ptr,
...
)
{
	...
	int ret = 0;
	ret = global_interesting_cl(g_container, x, fx, g, 0, mis->thread, search_depth, origin, box_size, global_ptr, count_id, torsion_size);
	...
	while (true) {
		...
		fxp = fx;
		 ...
		 // ls = line_search_zh(m_cl_gpu,
		    ls = line_search_lewisoverton_update(m_cl_gpu,
		 //  ls = line_search_zh_update(m_cl_gpu,
			p_cl_gpu,
			ig_cl_gpu,
			n,
			&step,
	...
	if (!global_flag) {
			add_to_global_buffer(g_container, mis->thread, search_depth, x, fx, g, origin, box_size, global_ptr, count_id, torsion_size);
		}
