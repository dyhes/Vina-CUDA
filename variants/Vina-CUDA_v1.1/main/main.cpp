					...
					tee &log, int search_depth, int thread, std::vector<std::vector<std::string>> ligand_names, int rilc_bfgs)
{
	...
	else
	{
		...
		for (int ligand_count = 0; ligand_count < ligand_num; ligand_count++)
		{
			...
			{
				// tmp_steps = unsigned((70 * 3 * (50 + (ms[ligand_count].num_movable_atoms() + 10 * ms[ligand_count].get_size().num_degrees_of_freedom()))) / 2); //for Autodock Vina global_steps
				tmp_steps = (0.24 * ms[ligand_count].num_movable_atoms() + 0.29 * ms[ligand_count].get_size().ligands[0] - 3.41) * 2.4;
			}
			if (tmp_steps < 1)
				tmp_steps = 1;
