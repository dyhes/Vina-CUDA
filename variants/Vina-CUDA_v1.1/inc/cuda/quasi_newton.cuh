...
__device__ flo ig_eval_deriv_update_optimized(		output_type_cl*		x,
											change_cl*			g, 
						const				flo				v,
											grids_cl*	grids,  // delete the constant
											m_cl*				m,
	...
	for (int i = 0; i < m->m_num_movable_atoms; i++) {
		...
		//e = e + g_evaluate(&grids->grids[t],  m_coords->coords[i], grids->slope, v, deriv, epsilon_fl);
		e = e + g_evaluate_update(&grids->grids[t], m_coords->coords[i], grids->slope, v, deriv, epsilon_fl,&grids_cuda->grid_other[t]);
...
__device__ flo eval_interacting_pairs_deriv(  pre_cl*			pre,  
									const				flo			v,
...
__device__ flo eval_interacting_pairs_deriv_update_optimized(  pre_cl*			 pre,  // delete the constant
									const				flo			v,
									const				lig_pairs_cl*    pairs,
									const			 	m_coords_cl*	 m_coords,
														m_minus_forces*  minus_forces,
...
__device__  void POT_deriv_optimized(	
				const					m_minus_forces* 	minus_forces,
				const					rigid_cl*			lig_rigid_gpu,
				const					m_coords_cl*		m_coords,
										change_cl*			g					
...
__device__ flo m_eval_deriv_optimized(		output_type_cl*		c,
										change_cl*			g,
										m_cl*				m,
										pre_cl*				pre,  //delete the constant 
										grids_cl*		grids, //delete the constant 
	...
	flo e = ig_eval_deriv_update_optimized(c,
							g, 
							v[1],				
							grids,
							m,
	...
	e += eval_interacting_pairs_deriv_update_optimized(	
										pre,
										v[0],
										&m->ligand.pairs,
										m_coords,
	...
	POT_deriv_optimized(minus_forces, m_gpu_rigid, m_coords, g);
...
__device__ inline void minus_mat_vec_product_optimized(	const		matrix_gpu*		h,
							const		change_cl*	in,
										change_cl*  out,
							const		int			lig_torsion_size,
							const		int			threadNumInBlock,
	...
	extern __shared__ flo shared_data[];

	for (int i = threadNumInBlock; i < n; i += threadsPerBlock) {
        shared_data[i] = find_change_index_read_update(in, i, lig_torsion_size);
    }
	__syncthreads();
	...
	for (int i = threadNumInBlock; i < n; i += threadsPerBlock) {
		...
		for (int j = 0; j < n; j++) {
			...
			sum += h->data[index_permissive(h, i, j)] * shared_data[j];
		}
		find_change_index_write_update(out, i, -sum, lig_torsion_size);
	}
}
...
__device__  flo line_search_optimized(					 	m_cl*				m,
							pre_cl*				pre, // delete the constant
							grids_cl*			grids, // delete the constant
										int					n,
					const				output_type_cl*		x,
	...
	for (int trial = 0; trial < max_trials; trial++) {
		...
		*f1 = m_eval_deriv_optimized(x_new,
								g_new,
								m,
								pre,
								grids,
...
__device__ bool bfgs_update_optimized(			matrix_gpu*			h,
					const	change_cl*		p,
					const	change_cl*		y,
					const	flo			alpha,
					const	mis_cl*			mis,
	...
	extern __shared__ flo shared_minus_hy[];
	extern __shared__ flo shared_p[]; 

	for (int i = threadNumInBlock; i < n; i += threadsPerBlock) {
        shared_minus_hy[i] = find_change_index_read(&minus_hy, i, s);
		shared_p[i] = find_change_index_read(p,i,s);
    }
    __syncthreads();

	for (int i = 0; i < n; i++) {
		for (int j = i; j < n; j++) {
			flo tmp = alpha * r * (shared_minus_hy[i] * shared_p[j]
									+ shared_minus_hy[j] * shared_p[i]) +
									alpha * alpha * (r * r * yhy + r) *  shared_p[i] * shared_p[j];
...
__device__ void bfgs_optimized(	output_type_cl*			x,
								change_cl*			g,
								m_cl*				m,
								pre_cl*				pre,  // delete the constant 
								grids_cl*			grids,  // delete the constant 
...
) 
{
	...
	flo f0 = m_eval_deriv_optimized(x,
						g,
						m,
						pre,
						grids,
	...
	for (int step = 0; step < max_bfgs_steps; step++) {
		...
		const flo alpha = line_search_optimized(	m,
										pre,
										grids,
											n,
											x,
